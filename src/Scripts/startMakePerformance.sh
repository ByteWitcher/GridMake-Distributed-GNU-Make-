...
JOB_ID=$(ssh $SITE "oarsub -p "cpuarch='x86_64'" -l host=$NUM_HOSTS,walltime=$WALLTIME 'source ~/src/Scripts/runMakePerformance.sh'" | grep "OAR_JOB_ID" | cut -d= -f2)
...
scp $SITE:~/make-performance-log.txt $SCRIPT_PATH/Results/Logs/
scp $SITE:~/make-performance-results.txt $SCRIPT_PATH/Results/
